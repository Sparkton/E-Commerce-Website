 ...
 * 
 * import com.godrej.dao.ProductDao; import com.godrej.daoimpl.ProductDaoImpl;
 * import com.godrej.model.Product;
 * 
 * @WebServlet("/PdtLogin") public class PdtLogin extends HttpServlet { private
 * static final long serialVersionUID = 1L; protected void
 * doPost(HttpServletRequest request, HttpServletResponse response) throws
 * ServletException, IOException { ProductDao pdtDao = new ProductDaoImpl();
 * String uName = request.getParameter("pName"); List<Product> tempList = new
 * ArrayList<Product>(); tempList = pdtDao.search(1,uName);
 * if(tempList.isEmpty() || tempList == null)
 * response.sendRedirect("jsp/Login.jsp"); for(Product user:tempList) {
